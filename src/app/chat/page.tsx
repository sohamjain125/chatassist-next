...
export default function Chatbot() {
    ...
    try {
      ...
      if (data.success) {
        ...
        if (data.error === 'Access denied') {
          ...
          toast({
            title: "Error",
            description: data.error || "Failed to load chat history",
            variant: "destructive",
          });
        }
        setMessages([]);
  ...
  // Add a useEffect to handle immediate navigation for unauthorized access
  useEffect(() => {
    const checkAccess = async () => {
      if (!hash || !sessionId) return;
      ...
      try {
        ...
        if (!data.success && data.error === 'Access denied') {
          setIsNavigating(true);
          router.push('/dashboard');
          toast({
            title: "Access Denied",
    ...
    checkAccess();
  }, [hash, sessionId, router, toast, isNavigating]);
            ...
            <div className="h-[calc(100vh-20rem)] flex items-center justify-center">
              <div className="text-center">
                ...
                <p className="text-muted-foreground">
                  {isNavigating ? "Redirecting..." : "Validating chat session..."}
                </p>
              </div>
            </div>
          ) : (
